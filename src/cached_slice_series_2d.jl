mutable struct CachedSliceSeries2D{T,N,AA<:AbstractArray} <: CachedSeries2D{T,T,N,AA}
    parent::AA
    cached::Array{T,2}
    cache_idxs::Tuple
end
...
axes(A::CachedSliceSeries2D) = axes(A.parent)
...
function CachedSliceSeries2D(A::AbstractArray{T,N}) where {T,N}
    ...
    cache_rngs = Base.tail(Base.tail(axes(A)))
    cache_idxs = map(first, cache_rngs) #(ones(Int, N-2)...)
    s = CachedSliceSeries2D{T,N,typeof(A)}(A, cached, cache_idxs)
